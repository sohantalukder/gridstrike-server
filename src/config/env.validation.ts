...
export function pickEnvConfig(
  config: Record<string, unknown>,
): Record<string, unknown> {
  return Object.fromEntries(
    ENV_KEYS.filter((key) => config[key] !== undefined).map((key) => [
      key,
      config[key],
    ]),
  );
...
export class EnvironmentVariables {
  ...
  @IsUrl({ require_tld: false }, { message: "SCENARIO_ASSET_BASE_URL invalid" })
  @IsOptional()
  SCENARIO_ASSET_BASE_URL?: string;
  ...
  @IsString()
  @IsIn(["prod", "local", "dev"], {
    message: "NODE_ENV must be prod, local, or dev",
  })
  NODE_ENV = "prod";
}
